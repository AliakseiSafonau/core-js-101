...
function getDigitalRoot(num) {
  let r = num;
  while (r > 9) {
    r = String(r).split('').reduce((acc, cuv) => acc + Number(cuv), 0);
  }
  return r;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < s.length; i += 1) {
    for (let j = 0; j < conditions.length; j += 1) {
      if (s.length === 4 || s.length === 8) {
        if (conditions[j][1] === s[i] && conditions[j][0] === s[i - 2]) result = false;
      }
    }
  }
  ...
  while (s.length) {
    ...
    for (let c = 0; c < conditions.length; c += 1) {
      if (conditions[c][0] === s[0]) {
        const re = new RegExp(`[\\${conditions[c][1]}\\${conditions[c][0]}]`, 'g');
        s = s.replace(re, '');
      }
    }
  }
...
function getCommonDirectoryPath(pathes) {
  return pathes.reduce((acc, cuv) => {
    const result = [];
    const accS = acc.split('/');
    const cuvS = cuv.split('/');
...
function evaluateTicTacToePosition(position) {
  ...
  result.push([position[0][0], position[1][0], position[2][0]]);
  result.push([position[0][1], position[1][1], position[2][1]]);
  result.push([position[0][2], position[1][2], position[2][2]]);
  result.push([position[0][0], position[1][1], position[2][2]]);
  result.push([position[0][2], position[1][1], position[2][0]]);
  for (let i = 0; i < result.length; i += 1) {
    if (result[i][0] && result[i][1] && result[i][2]) {
      if (result[i][0] === result[i][1] && result[i][0] === result[i][2]) {
        [answer] = result[i];
        break;
      }
    }
